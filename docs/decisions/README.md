...
- [ADR-0061](0061-runtime-global-load-control.md)：Runtime 全局负载控制 —— 只保留一个跨全部项目/Adapter 的并行上限（默认 2、范围 1–16，旧显式值取最小值迁移）；全局暂停 = 持久启动屏障 + 按 `pid + start token + incarnation` 可核验的 Provider 主进程冻结（不改 Task 状态、不向工具子进程发停止信号、跨重启保持，只有显式继续才解除）。**Amends ADR-0030/0032/0033 的容量层级**。**容量上半已实现**（FOUNDATION-096，schema v34：`runtime_capacity_settings`、全局事件 `project_id = NULL`、命令面 `scheduler capacity get|set|reset`）；**全局暂停仍属已接受、尚未实现**（`scheduler control status|pause|resume|reconcile`、持久屏障与 Provider 冻结）。
...
- **全局负载控制**：ADR-0061 —— 一个 Runtime 只有一个跨项目并行上限（**实现事实**：FOUNDATION-096，schema v34，命令面 `scheduler capacity get/set/reset`，项目级/Adapter 级覆写已退役）；全局暂停 = 持久启动屏障 + 可核验 Provider 主进程冻结，不替 ADR-0016 的单 Task pause，也不自动跨重启恢复（**尚未实现**）。当前没有全局暂停控制面（`pauseState` 只能是 `RUNNING`）。
...
| Phase 2 | Runtime 全局负载控制（唯一跨项目上限、全局暂停） | **上限已实现**（FOUNDATION-096：schema v34、`scheduler capacity get\|set\|reset`、跨项目按 Task 计数、全局事件）；**全局暂停仍未实现**：`scheduler control status\|pause\|resume\|reconcile`、跨重启持久屏障、三 Adapter 进程归属 spike 与 UI 投影都还没做；在定向验收前不得把暂停目标语义当作已交付 |

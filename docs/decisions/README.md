...
- [ADR-0045](0045-global-ui-settings.md)：五个全局界面效果设置存入 `$CODEESTRA_HOME/ui-settings.json`（损坏即报错，不静默回退）；CLI 命令面完备，不占迁移号。**命令面由 ADR-0067 暂停**；源码与已有文件保留但当前 Runtime 不读取。
...
- [ADR-0062](0062-automatic-worktree-reclamation-after-integration.md)：集成成功后自动回收该批成员的 Task worktree（默认开启，`settings auto-reclaim on|off` 可关闭；复用 `reclaim` 的同一套归属校验与 append-only 账本，失败现场仍默认保留；无 schema 变更）。**Superseded by ADR-0064**：它的唯一触发点 `task integrate` 已随集成一起删除，实现与设置项已移除；`reclaim` 仍是显式命令。
...
- [ADR-0076](0076-task-commands-address-a-task-by-id.md)：**`task` 命令以 Task id 为地址，project 是 Task 的字段**。`tasks.id` 是全局主键；这些 Runtime 请求的 `projectId` 变为可选，由 Runtime 在 dispatch 前从 Task/Operation/Delivery 行解析（未信任或不存在 = `NOT_FOUND`，两者不一致 = `TASK_PROJECT_MISMATCH`）；`task create` 改 `--project <id>`，`task list` 默认列出本 Runtime 全部已信任项目（`--project` 过滤），`task depends list` 取 `<task-id>` 或 `--project` 二者之一，`task schedule status|plan|run` 仍收 `<project-id>`；旧写法**硬切换**不再接受。**无 schema 变更**；**Amends** ADR-0005 的寻址拼写。

## 当前有效语义（与旧 ADR 冲突时按此执行）

- **目标与实现边界**：ADR-0070 S1–S4 已实现：Service/Process/Signal 持久内核、dispatcher/registry 与 `service/process/signal/intent` CLI；S5–S8 各已交付一个纵向切片：Process 完成写路径与只读进度（ADR-0071）、intention 结构化 outcome 路由与澄清内核事实（ADR-0072）、Project/Task **创建**单一写路径（ADR-0073）、**受管 integration（ADR-0074，schema v38）**。**仍不得声称**：原生 Agent-supervising Process 控制、真实模型意图解释、Attention 全局索引对 kernel 级 Intention 的接通、submit/revision/验证/取消/归档写路径已切换、Integration Process/Agent 或 eligibility 解耦（S9）。
...
- **Task 寻址（ADR-0076，已切换）**：`task` 组命令只收 `<task-id>`，project 是 Task 的字段；解析在 Runtime dispatch 之前一处完成（`apps/runtime/src/task-scope.ts`），storage 侧由 `resolveTaskProject` / `resolveOperationProject` / `resolveRevisionDeliveryProject` 与 `listAllTasks` 承担。**不提供旧写法兼容**；`task schedule status|plan|run` 与 `task create --project` 是刻意的例外。
...
- **Task 输入字段**：ADR-0065 —— `task create` 三个必填字段（显示标题 / 命名标题 / 任务详情）；两个标题是 Task 级、创建后不可修订；命名标题决定分支与 worktree 目录（`task/<编号>-<slug>`，旧任务仍用内部 ID）；约束与任务类型已彻底删除。
- **Task 寻址**：ADR-0076 —— `task` 组命令只收 `<task-id>`（全局唯一主键；project 是字段），Runtime 在 dispatch 前统一从 Task/Operation/Delivery 行解析项目，未信任或不存在 = `NOT_FOUND`，同时给出不一致的 `projectId` = `TASK_PROJECT_MISMATCH`；`task create` 用 `--project <id>`，`task list` 默列本 Runtime 全部已信任项目（`--project` 过滤），`task depends list` 取 `<task-id>` 或 `--project` 之一，`task schedule status|plan|run` 仍收 `<project-id>`；旧 `<project-id> <task-id>` 写法是用法错误（退出码 2），**没有兼容层**。本轮范围**只限 `task *` 组**：`project impact *` / `project.integration.request` / `session.guidance *` / `scheduler reservations *` / `reclaim --task` 的拼写未变。
...
- **资源回收**：ADR-0021/0037 —— 显式 `reclaim plan/apply/records`，只删归属校验通过的三类资源、默认保留失败现场、append-only 账本。ADR-0062 的「集成成功后自动回收」随 ADR-0066 删除集成而移除，没有自动路径。

以上各条都**不放宽**既有不变量：保留失败现场、不 `--force`、命令幂等、崩溃按事实收敛；也都不新增权限门禁或审批层。（「失败不动 `dev`」与「CAS 推进 ref」随 ADR-0066 删除 dev 集成而不再适用。）

## 待决项

| 阶段 | 尚需确认/验证 | 当前处理 |
|---|---|---|
| S8 | 发布出口：integration ref 如何交付到用户 main/release | 命名/初始化/CLI/基线已由 ADR-0074 关闭；发布出口仍不在 ADR-0070 与本 ADR 内，未定前不提供任何发布命令、不恢复 `promotion *` |
| S8 | Integration Process/Agent（复杂合并的模型辅助解决） | ADR-0074 D05 选 A：本轮不做，冲突如实报告并保留现场；要做需要 Project Git API + 预算 + 归属校验，另立 ADR |
| 任意（内核） | kernel 级 Intention 的澄清如何进入 Attention 全局索引 | `attention_requests.session_id` 是非空外键，无会话 Attention 不可表达；需要一次新 migration（ADR-0072 D01）。**v38 已由 S8 的受管 integration 占用且没有改这张表**，所以这件事的下一个空闲版本号尚未分配，未定前不做 |

...
- Pi 首版对敏感操作逐次审批。受控策略允许明确的只读操作；写文件、执行命令及其他敏感操作建立 Runtime Attention，未知工具 fail-closed。批准绑定具体 session/request/tool input，不是会话级永久授权。
...
敏感/只读分类必须版本化且 fail-closed。所谓“只读”不只依据工具名称；参数可能改变行为时仍需审批。Pi 原生 extension UI 负责承载请求，Codeestra 不向 Agent 注入伪造的自然语言同意。

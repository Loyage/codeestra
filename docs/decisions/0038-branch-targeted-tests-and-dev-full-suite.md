...
> **Amended by ADR-0039（FOUNDATION-065）**：本节第一句原本写「在 `dev` 工作树」运行。实现（`promotion.full-suite run --dev-commit <full-sha>`）由 Runtime 在**该精确候选 SHA 的隔离副本**上运行，而不是字面在 dev 工作树里。用户已确认改本节措辞以对齐实现：副本对「精确候选 SHA」的保证更强（不含工作树的未提交改动、被 gitignore 的本地状态与本地构建产物），并与 ADR-0006 的 detached 副本惯例一致；候选内容仍必须与 `dev` 的候选 SHA 完全一致。

- 所有待提升改动集成到 `dev` 后，对**精确候选 SHA** 运行一次全量测试——由 Runtime 在该 SHA 的隔离副本上运行并观察结果（字面的 dev 工作树不是必需的，也不作为证据来源）。

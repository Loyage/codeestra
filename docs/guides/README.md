...
> **ADR-0068 文档修订**：更新长期定位与目标架构入口；现有命令参考仍严格对应 schema v36。
...
> **当前实现边界**：本目录只描述 schema v36 **真实具备**的能力。ADR-0068 的 `service/process/signal/intent` 通用命令与受管 integration 尚未实现，因此不会提前写进命令参考。目标设计见 [Service Kernel 架构](../architecture/service-process-signal.md)。每条现有命令、参数、退出码与错误码都从仓库源码核对得到

...
|---|---|---|---|---|
| 项目识别 | 读仓库身份：工作树根、`main` ref、对象格式、HEAD，以及**dev clone 的** `dev` ref/commit 与只读的本地 `dev` ref 退役证据（`devRefRetirement`） | `project inspect [path] [--dev-repo <dev-clone>]` | 项目 → 添加本地项目（主路径 + **dev clone 路径（必填）**输入后） | [0047](../decisions/0047-github-mediated-stable-promotion.md)、[0056](../decisions/0056-dev-repo-path-single-dev-fact-source.md) |
...
| 项目接入（trust） | 注册项目；把「你刚看到的身份 + 验证策略 digest + 影响映射 digest + dev clone 核验结果」一起确认；**`--dev-repo` 必需**（省略或 `none` 以 `DEV_REPO_REQUIRED` 拒绝且不写入任何行）；FULL 零确认 / STRICT 输 `TRUST` | `project trust [path] --dev-repo <dev-clone> [--yes]` | 项目 → 添加/信任此项目（**dev clone 路径必填，为空时按钮不可点**；被拒绝时显示稳定码 + 本地解释） | [0011](../decisions/0011-default-full-permission-mode.md)、[0031](../decisions/0031-impact-snapshot-and-deterministic-conflict-analyzer.md)、[0047](../decisions/0047-github-mediated-stable-promotion.md)、[0056](../decisions/0056-dev-repo-path-single-dev-fact-source.md) |
| dev 事实的唯一来源 | 长期 `dev` 分支、Task 基线/worktree/branch、集成 worktree 与 ref 推进、回收归属、提升候选对象与全量证据的副本/锁文件都来自 `projects.dev_repo_path`；未记录时以 `DEV_REPO_REQUIRED` 拒绝，绝不回退到某个 clone 自己的本地 `dev` ref | 所有需要 dev 基线的命令（`task *`、`reclaim *`、`promotion full-suite run` 等） | —（同一命令面） | [0056](../decisions/0056-dev-repo-path-single-dev-fact-source.md) |
...
| 永久删除 | **不可撤销**：删掉任务的全部记录（含 append-only 的修订/impact/定向测试计划/知识绑定）与它自己的 worktree、验证副本、`task/<id>` 分支，并写一条 `TaskPurged`；需 `--yes`；非终态先协作停止；**成果已进 `dev`/`main` 的任务拒绝**（只能归档） | `task purge <project> <task> <expected-version> --yes [--reason <text>]` | 任务详情 →「更多操作」→「永久删除」（输入任务编号才启用） | [0058](../decisions/0058-task-purge.md) |

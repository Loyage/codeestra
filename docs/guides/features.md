...
| 影响映射校验 | 报告 `main` ref 上的 `.codeestra/impact.json` 是否存在且是已确认的那一份 | `project impact validate [path] [--json]` | 项目 → 影响映射 | [0031](../decisions/0031-impact-snapshot-and-deterministic-conflict-analyzer.md) |
...
| 运行任务 | 显式请求启动；同自动调度同一门禁（依赖/冲突/容量） | `task run <project> <task> <expected-version> [--adapter <id>] [--allow-unknown] [--json]` | 任务详情 → 运行任务 | [0030](../decisions/0030-phase2-parallel-scheduling.md) |
| 暂停 / 恢复 | 暂停是协作停止（确认 provider 退出后才 `PAUSED`）；恢复以 provider conversation resume 继续 | `task pause`、`task resume <…> [--adapter <id>] [--allow-unknown]` | 任务详情 → 暂停 / 恢复 | [0016](../decisions/0016-task-pause-cancel-archive.md) |
| 重试失败任务 | 只对 `FAILED` 生效；重新入队（可能需要先重建 worktree）后，用同一个门禁请求一次启动 | `task retry <project> <task> <expected-version> [--adapter <id>] [--json]` | 任务详情 → 重试 | [0036](../decisions/0036-task-retry-after-failure.md)、[0042](../decisions/0042-rebuild-reclaimed-worktree.md) |
...
| 规格修订 | 创建新 revision（可只改理由、只加约束）并列出历史 | `task revision create`、`task revision list` | 任务详情 →「更多操作」 | [0028](../decisions/0028-revision-delivery-and-stale-session-startup-reconcile.md) |
| Revision 投递台账 | 单独读取与解决「修订是否真的到达运行中的 Execution」 | `task revision delivery list/get/resolve` | 任务详情 → 修订投递 | [0028](../decisions/0028-revision-delivery-and-stale-session-startup-reconcile.md) |
...
| Agent 配置 | 持久化 provider/model/thinking，分全局默认与每项目覆盖；逐字段按 `环境变量 > 项目 > 全局 > Adapter 默认` 解析；只影响新 Session | `agent config get/set/clear [--project <id>] [--adapter <id>] [--provider/--model/--thinking/--unset]` | Agent 配置标签页 | [0012](../decisions/0012-agent-configuration-scopes.md) |
| 多 Adapter | 注册 `pi`（默认）、`codex`、`claude`；每次运行绑定一个 Agent，换 Adapter 是新建 Execution | `task run/resume/retry --adapter <id>` | 任务详情 → 运行任务（选择 Agent） | [0029](../decisions/0029-codex-adapter-transport-and-capabilities.md)、[0040](../decisions/0040-claude-code-adapter-transport-and-capabilities.md) |
...
| 影响分析与冲突判定 | 确定性地把 change set 映射到 `.codeestra/impact.json`，与所有持有资源的 Task 比较，得出 `SAFE_TO_PARALLELIZE / UNKNOWN / CONFLICTING` 及交叉路径 | `project impact validate/show/explain` | 项目 → 影响映射；任务详情 → 影响与冲突判定 | [0031](../decisions/0031-impact-snapshot-and-deterministic-conflict-analyzer.md) |
...
| 分层测试证据 | 分支把定向范围写进 `.codeestra/tests.json`；`record` 把它快照成绑定 `(task, revision, commit, digest)` 的 append-only 记录；`verify` 只消费已记录的计划 | `task tests record/show/history` | 任务详情 → 验证策略来源 | [0038](../decisions/0038-branch-targeted-tests-and-dev-full-suite.md)、[0039](../decisions/0039-layered-verification-evidence.md) |
...
| 能力 | 能做什么 | CLI 入口 | UI 位置 | ADR |
|---|---|---|---|---|
| dev 全量测试证据 | 在精确 dev SHA 的 detached 副本上运行项目固定策略，**Runtime 观察结果**，客户端不能自报；证据绑定 SHA/策略 digest/锁文件 digest | `promotion full-suite run --dev-commit <full-sha>`、`promotion full-suite list` | 任务详情 → 稳定提升记录 | [0038](../decisions/0038-branch-targeted-tests-and-dev-full-suite.md)、[0039](../decisions/0039-layered-verification-evidence.md) |
| 提升 prepare/approve/promote | `prepare` 固定三元组（不写 Git）；`promote` 在 main 工作树里 fast-forward 并跑 install/build/stop/status；重启仅在每步退 0 且 Runtime 回答 READY 时记录 | `promotion prepare/approve/promote/abandon/get/list` | 任务详情 → 稳定提升记录 | [0009](../decisions/0009-main-dev-promotion-and-restart.md)、[0022](../decisions/0022-stable-branch-promotion.md) |
...
| Project Knowledge | 分层知识（人工 `instructions`/`skills` 从 `main` ref 读 + Runtime 数据目录里的机器生成层）；无覆盖语义、重复 id/路径 fail-closed；逐条来源与 digest 进快照 | `project knowledge validate/list/show/resolve` | 项目 → 影响映射旁（只读展示）、任务详情 | [0041](../decisions/0041-project-knowledge-layers-and-execution-binding.md) |
...
| 设置 | 散文提问等待的全局开关（`auto` / `record-only` / `off`）；读写同一命令，无需确认 | `settings prose-question-attention [mode]` | —（CLI-only） | [0043](../decisions/0043-prose-question-attention-escalation.md) |
...
| Runtime 生命周期 | 单实例、自动拉起、两阶段 stop 与 ownership 报告 | `status`、`stop [--wait <s>]`、`permission get` | 顶部运行状态 | [0025](../decisions/0025-runtime-lifecycle-stop-and-single-instance.md) |
| 界面主题 | 亮/暗主题切换（纯前端偏好，不改任何业务语义） | —（无 CLI 语义；主题不进入命令面） | 顶部主题选择器 | [0015](../decisions/0015-task-workbench-and-themes.md) |
...
6. 文档与实现不一致的地方在 [troubleshooting.md](./troubleshooting.md) 与
   `docs/tasks/README.md` 的 FOUNDATION-070 一节里**如实列出**，未做静默改写。

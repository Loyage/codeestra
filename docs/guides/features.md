...
|---|---|---|---|---|
| 项目识别 | 读仓库身份：工作树根、`main` ref、对象格式、HEAD，以及**dev clone 的** `dev` ref/commit 与只读的本地 `dev` ref 退役证据（`devRefRetirement`） | `project inspect [path] [--dev-repo <dev-clone>]` | 项目 → 添加本地项目（路径输入后） | [0056](../decisions/0056-dev-repo-path-single-dev-fact-source.md) |
...
| 项目接入（trust） | 注册项目；把「你刚看到的身份 + 验证策略 digest + 影响映射 digest + dev clone」一起确认；**`--dev-repo` 必需**（省略或 `none` 以 `DEV_REPO_REQUIRED` 拒绝且不写入）；FULL 零确认 / STRICT 输 `TRUST` | `project trust [path] --dev-repo <dev-clone> [--yes]` | 项目 → 添加/信任此项目 | [0011](../decisions/0011-default-full-permission-mode.md)、[0031](../decisions/0031-impact-snapshot-and-deterministic-conflict-analyzer.md)、[0056](../decisions/0056-dev-repo-path-single-dev-fact-source.md) |

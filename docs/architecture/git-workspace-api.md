...
| `TrustedProject.devRepoPath` | dev clone，或 null（managed，ADR-0060） | dev 事实的入口；为空时 Task 基线改取项目文件夹的检出分支。**ADR-0060 第三轮修订后只有集成与提升**（`task integrate`、`promotion *`、`promotion full-suite run`）以 `DEV_REPO_REQUIRED` 拒绝 |
| `taskWorkspaceRepositoryRoot(project)`（Runtime 辅助，`devRepoPath ?? repoRoot`） | **dev clone，或项目文件夹**（`COALESCE`，ADR-0060） | 结果 commit 的 worktree 归属校验（`assertOwnedWorkspace`）、`task retry` 的 worktree 观测、`task purge` 的 Task 分支删除、`task verify` 已由 `VerificationCandidates.repositoryRoot` 覆盖 |
...
| `VerificationCandidates.repositoryRoot` | **dev clone，或项目文件夹**（`COALESCE`，ADR-0060；被验证的 commit 是那个仓库里的对象） | 验证副本的创建、`testedCommit` 的 tree/计划文件读取 |
...
4a. **ADR-0047 唯一提升路径（已实现，schema v29）**：本机 `main` 与 `dev` 是**两个分别 clone 的独立仓库**（ADR-0048），稳定提升不再由 Runtime 在 main 工作树内 ff 本地 `dev` ref。产品只执行「push 固定候选到远端 `dev` + 读回核对」并报告**「已推送、等待拉取」**（`phase: AWAITING_PULL`，退出码 3），main 检出的 `git fetch` + `git merge --ff-only origin/dev` 是**用户显式步骤**；再次调用同一命令核对到 main 检出已在候选上后，才记录重启序列、跑它、并在重启核对成功后把候选 push 回远端 `main`（`phase: MAIN_PUSH_PENDING` 期间不得报告完成）。旧的本机 `fastForwardCheckedOutWorktree` 已删除，不存在双路径。
   ...
   - **dev clone**：`projects.dev_repo_path` 记录第二个 clone，它是**长期 `dev` 分支**（集成与提升）的事实来源。**ADR-0060 起它由必需改为可选**（第三轮修订后需要它的只剩 `task integrate`、`promotion *` 与 `promotion full-suite run`；`project trust` 省略或写 `none` 时按 managed 记录，`DEV_REPO_REQUIRED` 不再由 trust 返回），运行时**绝不回退**到某个 clone 自己的本地 `dev` ref。`project trust --dev-repo <path>` 逐条核验它（是 Git work tree、是**另一个** clone 而非 main 检出或其 worktree、`origin` 与 main 检出一致、HEAD 在项目 `dev` 分支上且该分支存在），不可核验即用稳定码 `DEV_REPO_*` 拒绝且**不写入**；`project inspect [--dev-repo <path>]` 报告同样的核验结果与 `clean`，并额外报告只读的 `devRefRetirement`（被检查检出自己的本地 `dev` ref 状态 + 仍没有 dev clone 的已信任项目）。push 前另核对候选对象在该 clone 中存在（`DEV_REPO_CANDIDATE_MISSING`）。候选与副本根/锁文件从 **dev clone** 读，全量策略仍从 **main ref** 读。

...
interface GitWorkspacePort {
  ...
  captureResult(input: {
    ...
    expectedHead: string; authorizedPaths: readonly string[];
...
- prepare 以固定 main SHA 为基线，独立 `refs/heads/task/<stable-id>-<safe-slug>` 与 `.codeestra/worktrees/<stable-id>/`。只把 slug 当显示辅助，不把用户文本当 ref/path。
...
- captureResult 先确认 Agent 与工具静止，检验当前 revision/HEAD/diff。只提交经执行策略授权的任务成果，禁止 `git add .` 把密钥、日志、SQLite、其他文件一并打包。
- 如果 Agent 已创建成果 commit，核对可达关系和差异后固定该 OID；Runtime 不重写其历史。自动为尚未提交 diff 创建 commit 的授权、身份与 hook 行为在 Phase 1 Git spike 中确认，未经确认时返回需要用户提交/批准，不擅自写 commit。
...
独立 worktree 不隔离 git config、hooks、对象库、凭据和操作系统权限；hooks/filters/子模块可能执行代码或访问网络。首次项目接入必须清晰展示信任边界；不偷偷禁用用户 hooks，也不默默运行未知 hooks 来声称验证安全。

## 4. 崩溃恢复与测试

Operation 在副作用前写 PLANNED；外部资源带 operation/ownership identity。恢复先 `git worktree list`、查 refs 与 HEAD、查进程归属，再决定补记成功或请求人工处理。

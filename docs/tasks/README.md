...
1. 在 Runtime 接入 Pi adapter registry、事件 pump 与 answer 投递循环（含 CLI `task run`/attention 自动投递），并接入 outbox 长连接。
2. 实现 ADR-0003 的 ChangeSet、一次性成果 commit 确认与 Task verification；所有 Git 测试只使用临时仓库。

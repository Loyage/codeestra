...
状态：架构设计基线；八项关键决策已确认，Phase 0 纯领域工程可开始。Phase 1 真实 Runtime 仍须通过协议验证与 Git 授权准入，后续阶段待决项见决策索引。
...
7. 每个运行中 Task 独占 branch 和 worktree；不允许多个 Task 操作同一工作目录。例如 `task/T102-map-generation`、`.codeestra/worktrees/T102/`。
...
13. IntegrationBatch 是正式领域对象，记录任务集合、对应 revision、commit、基线、集成结果和验证证据。
14. Human-authored knowledge 和 machine-generated knowledge 分离；Agent 不能静默覆盖人工维护的知识文件。
15. Self Task 原则上可修改全部 Codeestra 源码，但只能在隔离开发环境形成 Candidate。运行中的 Stable 不被直接覆盖；Promotion 必须由用户发起。
16. 独立且极小的 `codeestra-bootstrap` 提供 list versions、launch version、switch version、health check、rollback，作为恢复入口。
...
└── worktrees/      # 运行产物；非项目知识
```

知识目录与运行数据的 Git 跟踪策略另行明确，不能将整个 `.codeestra/` 一概当作知识或一概忽略。密钥、终端日志和运行数据库不得意外进入提交。
...
目标是本机单用户开发编排。不引入 Kubernetes、Kafka、RabbitMQ、微服务拆分或分布式基础设施。采用独立本地 Runtime，桌面作为可重连客户端；关闭窗口不终止任务和 Session。Phase 1 首个真实 Adapter 为 Pi，保留原生权限审批，不自动放行；实际协议能力必须验证。取消采用协作停止，超时请求人工处理并保留资源；优先级只影响后续调度、不抢占。
...
当前仅进入 Phase 0 的领域模型与测试骨架。不实现：全部 Adapter、并行调度执行、完整桌面交互、自动集成发布、机器知识生成、自我升级、远端 Agent、多用户、多机器调度及分布式运行。Phase 1 真实 Pi 与 Git 副作用待技术和授权门禁通过后再实现。

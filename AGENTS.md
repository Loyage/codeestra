...
- Scheduler、Service registry、Signal dispatcher、Git workspace、Agent adapter、Verification、Integration 明确分层；每个活动 Process 仅一个主 Agent；每个运行 Task 独占 branch/worktree。
...
- 合入本仓库 `dev` 始终是人工 Git 动作：功能完成后由人把成果合回 `dev`；未来产品 managed integration 也不得用于本仓库自身发布。`dev` 合入 `main` 遵守上节规程。

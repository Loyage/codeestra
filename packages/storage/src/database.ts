...
export interface StoredEventEnvelope {
  ...
  /**
   * The Project this fact belongs to, or `null` for a Runtime-global fact (ADR-0061 D10, schema
   * v34). A global capacity or pause decision is not any one Project's event, so it is not
   * attributed to one; a Project-filtered reader receives both its own events and the global ones.
   */
  ...
  private migrate(): void {
    ...
    try {
      this.sqlite.transaction(() => {
        ...
        // Version 34 is this branch's own number (FOUNDATION-097 / ADR-0061, the pause half of a
        // schema version another branch also claims). The two halves are merged into one
        // `if (version < 34)` step at integration time; on this branch the step adds the persistent
        // global control tables and makes `domain_events.project_id` nullable so a Runtime-global
        // event is not attributed to a Project.
        if (version < 34) {
          // The `domain_events` rebuild is guarded by an explicit row-count comparison before and
          // after the copy. Bun's `exec()` swallows a *step-time* error inside a multi-statement
          // script and keeps going, so if the `INSERT ... SELECT` ever failed the following
          // `DROP TABLE` would still run and the event log would be gone without an error. The
          // comparison turns that into a loud rollback, exactly like the v28/v30 rebuilds above.
          const eventsBefore = this.sqlite.query<{ rows: number }, []>(
            'SELECT COUNT(*) AS rows FROM domain_events').get()?.rows ?? 0;
          this.sqlite.exec(runtimePauseControlMigration);
          this.sqlite.exec(domainEventsGlobalProjectMigration);
          const eventsAfter = this.sqlite.query<{ rows: number }, []>(
            'SELECT COUNT(*) AS rows FROM domain_events').get()?.rows ?? -1;
          if (eventsAfter !== eventsBefore) {
            throw new StorageError('INVALID_STATE',
              `Schema v34 rebuild of domain_events lost rows (${eventsBefore} before,`
              + ` ${eventsAfter} after); the upgrade was rolled back and nothing was changed`);
          }
          // The row count alone cannot see every failure: `exec()` swallows a step-time error inside
          // a multi-statement script, so a statement that failed *after* the copy would leave the
          // counts equal while the schema stayed half-built. These two reads assert the end state the
          // step promised, which turns exactly that case into a loud rollback.
          ...
          if (globalColumn === undefined || globalColumn.notnull !== 0) {
            throw new StorageError('INVALID_STATE',
              'Schema v34 promised a nullable domain_events.project_id and the rebuilt table does'
              + ' not have one; the upgrade was rolled back and nothing was changed');
          }
          ...
          if (controlRows !== 1) {
            throw new StorageError('INVALID_STATE',
              'Schema v34 promised one runtime_pause_control singleton row; the upgrade was rolled'

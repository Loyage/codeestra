...
interface TaskSummaryRow {
  ...
  readonly naming_title: string | null;
  readonly state: TaskLifecycleState;
  readonly priority: number;
  readonly version: number;
...
export class Phase1Database {
  ...
  private migrate(): void {
    ...
    // `domain_events` (v34), `tasks`/`task_revisions` (v35) and `projects` (v36) are each referenced
    // by name from other tables, so every upgrade below the newest such step runs with foreign keys
    // off and verifies the whole schema before the connection is used.
    const rebuildsTable = version < 36;
    ...
    try {
      this.sqlite.transaction(() => {
        ...
        // Version 36 is this step's own number (ADR-0066): the product stops modelling a dev clone,
        ...
        // rows, which this step does not touch. It runs after v35 because it backfills
        // `workspaces.base_ref` from `projects.dev_ref`, which only this step removes. No earlier
        // number is ever inserted.
        if (version < 36) {
          ...
          if (projectsAfter !== projectsBefore) {
            throw new StorageError('INVALID_STATE',
              `Schema v36 rebuild of projects lost rows (${projectsBefore} before, ${projectsAfter}`
              + ' after); the upgrade was rolled back and nothing was changed');
          }
        }

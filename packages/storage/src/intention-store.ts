...
import { DomainError } from '@codeestra/domain';
...
/** The S5 Process write path. Required: this store must never write Process state itself. */
...
export class IntentionStore {
  ...
  /**
   * The one write path for a Process's own state: it delegates to the S5 kernel method
   * (`ServiceKernelStore.transitionProcess`) and never writes `processes.status` itself. A Process has
   * exactly one writer, so an unwired kernel is a loud failure rather than a second implementation.
   */
  transitionProcess(input: {
    readonly processId: string;
    readonly expectedVersion: number;
    ...
    if (typeof delegate !== 'function') {
      throw new KernelStorageError('PROCESS_STATE_UNAVAILABLE',
        'The Runtime kernel does not expose the Process write path this store depends on');
    }

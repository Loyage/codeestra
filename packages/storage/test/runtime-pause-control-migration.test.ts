...
describe('schema v34 pause half (integrated with the capacity half)', () => {
  test('the pause half owns two tables and repeats neither the shared receipts table nor the event rebuild', () => {
    ...
    // The integration rule for one shared version number: `runtime_command_receipts` and the
    // `domain_events` rebuild exist exactly once in the schema, and the *capacity* half owns both
    // (FOUNDATION-096). A second definition here would be a silent duplicate of the same table.
    expect(runtimePauseControlMigration).not.toContain('runtime_command_receipts');
    expect(runtimePauseControlMigration).not.toContain('domain_events');
    ...
    // Both halves' global commands share one receipts table, which the capacity half creates.
    expect(runtimeGlobalCapacityMigration).toContain('runtime_command_receipts');
    // One `domain_events` rebuild for the one version number, and it makes project_id nullable.
    expect(runtimeGlobalCapacityMigration).toContain('domain_events_v34');
    expect(runtimeGlobalCapacityMigration).toContain('project_id TEXT REFERENCES projects(id)');
    // The retired project-scoped capacity configuration is dropped in that same step.
    expect(runtimeGlobalCapacityMigration).toContain('DROP TABLE project_capacity_limits');
  });
  ...
  test('a version 33 file database upgrades in place, keeps its events and makes project_id nullable', () => {
    ...
    // All four v34 tables exist after one upgrade, and the two tables the version retires are gone:
    // "four tables present" is the check the integration contract names, and it can only pass if the
    // two halves really ran in the one step.
    expect(upgraded.sqlite.query<{ name: string }, []>(`
      SELECT name FROM sqlite_master WHERE type='table'
      AND name IN ('runtime_capacity_settings','runtime_command_receipts','runtime_pause_control',
        'runtime_pause_targets') ORDER BY name`).all().map((row) => row.name)).toEqual([
      'runtime_capacity_settings', 'runtime_command_receipts', 'runtime_pause_control',
      'runtime_pause_targets',
    ]);
    expect(upgraded.sqlite.query<{ name: string }, []>(`
      SELECT name FROM sqlite_master WHERE type='table'
      AND name IN ('project_capacity_limits','project_adapter_slot_limits')`).all()).toEqual([]);
    // The shared receipts table has both halves' documented shape, with no extra column.
    expect(columns(upgraded.sqlite, 'runtime_command_receipts').map((column) => column.name))
      .toEqual(['command_id', 'payload_hash', 'result_json', 'created_at']);
    // No capacity row means "never set explicitly" (default 2); the pause singleton always exists.

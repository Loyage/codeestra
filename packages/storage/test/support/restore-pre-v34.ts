...
/**
 * Puts a fixture database back into the shape a pre-v34 database had (FOUNDATION-096 / ADR-0061).
 *
 * Several migration tests build their fixture by creating the *current* schema and then removing
 * exactly what the step under test added. Since schema v34 retires two tables and creates two new
 * ones, a fixture that claims to be v26/v28/v29 must also have the retired configuration tables back
 * and must not already hold the v34 tables — otherwise the upgrade it exercises is not the statement a
 * real database of that age runs.
 *
 ...
 * `runtime-capacity-migration.test.ts`, which builds its history from the migration constants.
 */
export function restorePreV34CapacitySchema(database: Database): void {
  database.exec(`

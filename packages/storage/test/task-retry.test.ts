...
function seedTask(target: Database, options: {
  ...
  target.transaction(() => {
    target.query(`INSERT INTO tasks
      ...
      VALUES ('t1','p1',1,'Do work','do-work','r1',?1,4,2,2,?2)`).run(options.state,
...
describe('task retry', () => {
  ...
  test('adds the version 23 columns without a retry table', () => {
    expect(phase1SchemaVersion).toBeGreaterThanOrEqual(23);

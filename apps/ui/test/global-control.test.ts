...
describe('the global control bar lives in the shell, not in a project page', () => {
  ...
  it('titles the capacity card as the Runtime-global capacity and states the scope it shows', () => {
    ...
    expect(source).toContain('data-capacity-scope={scope}');
    expect(source).toContain('data-capacity-scope-note="PROJECT"');
    // A project-scoped table may never be labelled as the global one.
    expect(source).toContain("'项目内（本次构建的容量命令面）'");
    // ...and the panel must say why, naming the command that owns the global numbers.
    expect(source).toContain('scheduler capacity get');
    expect(shell).not.toContain('data-capacity-scope="GLOBAL"');
  });

  it('renders the capacity table with its scope marker so project numbers cannot pass as global', () => {
    const markup = renderToStaticMarkup(createElement(CapacityTable, {
      capacity: {
        projectId: '11111111-1111-4111-8111-111111111111',
        globalLimit: 2,
        globalLimitSource: 'DEFAULT',
        globalUsed: 1,
        globalAvailable: 1,
        globalWaitReason: null,
        adapters: [],
        configVersion: 1,
        updatedAt: null,
        updatedBy: null,
    ...
    expect(markup).toContain('data-capacity-scope="PROJECT"');
    expect(markup).toContain('data-capacity-row="project"');
    expect(markup).toContain('项目内（本次构建的容量命令面）');
  });
});
